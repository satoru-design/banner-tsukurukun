    ...
    <section className="bg-white">
      <div className="max-w-6xl mx-auto px-4 sm:px-6 py-20 sm:py-24">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          {VOICES.map((v, i) => (
            <VoiceCard key={v.initials} {...v} offset={i === 1 ? 'md:translate-y-6' : ''} />
          ))}
        </div>
...
function VoiceCard({
  ...
  radius,
  offset,
}: (typeof VOICES)[number] & { offset?: string }) {
  return (
    <article className={`bg-stone-50 border border-slate-300 ${radius} p-6 flex flex-col gap-4 ${offset ?? ''}`}>
      ...
      <div>
        <span className="inline-flex items-center text-xs font-bold text-emerald-900 bg-emerald-900/[0.06] border border-emerald-800/30 px-3 py-1.5 rounded-full">
          {proofBadge}
        </span>
      </div>

      <p className="font-serif italic text-base text-emerald-900 leading-relaxed">
        「{catchphrase}」
      </p>

      <div className="bg-white border border-slate-300 rounded-[12px] p-4">
        <div className="mb-2">
          <span className="inline-block px-2 py-0.5 bg-slate-200 text-slate-700 rounded text-[10px] font-bold tracking-widest">
            BEFORE
          </span>
        </div>
        <p className="text-xs text-slate-700 leading-relaxed">{before}</p>
      </div>
      ...
      <div className="bg-emerald-900/[0.05] border border-emerald-800/30 rounded-[12px] p-4">
        <div className="mb-2">
          <span className="inline-block px-2 py-0.5 bg-emerald-800 text-white rounded text-[10px] font-bold tracking-widest">
            AFTER
          </span>
        </div>
