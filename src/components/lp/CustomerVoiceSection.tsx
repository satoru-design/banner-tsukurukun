...
const VOICES = [
  {
    ...
    after:
      '勝ちバナー作る君を導入してからは、思いついたキャッチコピーをその場で形にできるようになりました。デザイナーの手を煩わせることなく、数分でプロクオリティのバナーが何パターンも完成するため、AB テストの回数を以前より大幅に増やせています。制作の「待ち時間」がほぼなくなり、本来集中すべき数値分析や戦略立案に時間を割けるようになったのが一番の収穫です。',
  },
  {
    initials: 'T.S',
    ...
    after:
      'このサービスを使い始めてから、ルーチンワークの大部分を AI に任せられるようになりました。驚いたのはその精度の高さです。レイアウトの崩れが少なく、微調整だけで納品レベルに仕上がるため、リサイズ作業に費やしていた時間が大幅に短縮されました。空いた時間でサイト全体の UX 設計や新しい企画に集中できるようになり、仕事の質が劇的に向上しました。',
  },
  {
    initials: 'M.O',
    ...
    after:
      '勝ちバナー作る君を使い始めてから、商品写真をアップするだけでプロ級のバナーが瞬時に手に入るようになり、経営のフットワークが格段に軽くなりました。専門知識がなくても直感的に操作でき、その時のインプレッションを見て即座にクリエイティブを差し替えることができます。高額な外注費をかけずに広告施策が回せるようになり、少数精鋭の運営に欠かせないツールになっています。',
  },
];
