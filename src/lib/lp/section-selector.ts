...
import { LP_INDUSTRY_LABELS } from './types';
...
- 業種: ${industryLabel}
${brief.usp ? `- USP: ${brief.usp}` : ''}
- ターゲット: ${brief.target}
${brief.price ? `- 価格: ${brief.price}` : ''}
- オファー: ${brief.offer}
...
- 業種「${industryLabel}」がコスメ/サプリ/食品/健康/医療 → voice / pricing / numeric_proof 必須
- 業種が saas → comparison / features / pricing 必須
- 業種が education / 不動産 / 金融 → faq / voice / pricing 必須
- 業種が apparel → voice / pricing / numeric_proof
- 高単価商品（価格が "万円"以上）⇒ faq 必須
